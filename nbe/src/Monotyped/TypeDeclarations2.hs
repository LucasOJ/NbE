...
module Monotyped.TypeDeclarations2 (
    Ty(..),
    Elem(..),
    Expr(..),
    ClosedExpr,
...
reify (Base nf)    = nf
reify (Function f) = NormalLam (reify (f ope (evalVar (NeutralVar Head)))) 
    where
...
instance (SingContext xs, SingTy x) => SingContext (x:xs) where
    idOpe = Keep idOpe
    initialEnv = ConsEnv (strengthenEnv wk initialEnv) (evalVar (NeutralVar Head))
