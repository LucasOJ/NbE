...
Pattern matching on a type is possible with standard Haskell class instances. However, the automatic promotion of values to the type level by \code{DataKinds} extension enabled us to represent more complex structures such as lists (see Section \ref{sect:opes}) at the type level. Class instances worked seamlessly with \code{DataKinds} promoted types to pattern match at the type level (see Section \ref{subsect:typedEvalAppCase}), similarly to a dependent pattern match. 
