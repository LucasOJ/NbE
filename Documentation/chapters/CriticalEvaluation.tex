...
Richard Eisenberg's proposals for built-in dependent pattern matches using a syntax to similar Agda would be a much cleaner approach than \code{GADTs}, but the current solution for managing dependent pattern matches is the \code{singleton} library \cite{singletonLibrary}. This library uses template Haskell to automatically generate the instances and data types needed for “faking” dependent pattern matches, and would be useful for dependent pattern matching at scale.
