...
In this section implement NbE for the untyped lambda calculus in Haskell, following a mathematical specification by Lindley \cite{slides}. We begin by representing the sets that NbE will operate on as data types in Haskell, namely the term syntax, set of normal forms and semantic set. Once these sets are defined, we implement the \code{eval} and \code{reify} functions which compose to give the \code{normalise} function.
