...
Before using GADTs to construct the set of well-typed expressions, we first define the \code{Elem} GADT. A value of type \elemcode{Elem xs x} is a proof that the type \elemcode{x} is an element of the list of types \elemcode{xs}. 
